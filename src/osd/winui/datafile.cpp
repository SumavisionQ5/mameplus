...
int load_driver_mameinfo (const game_driver *drv, char *buffer, int bufsize)
{
	...
	*buffer = 0;
	...
	if (drv->flags &
	    ( MACHINE_NOT_WORKING | MACHINE_UNEMULATED_PROTECTION | MACHINE_IMPERFECT_GRAPHICS | MACHINE_WRONG_COLORS |
	      MACHINE_IMPERFECT_COLORS | MACHINE_NO_SOUND | MACHINE_IMPERFECT_SOUND | MACHINE_NO_COCKTAIL | MACHINE_REQUIRES_ARTWORK))
	{
		...
		if (drv->flags & MACHINE_REQUIRES_ARTWORK)
	...
	if (clone_of && !(clone_of->flags & MACHINE_IS_BIOS_ROOT))
	{
		strcat(buffer, _("\nORIGINAL:\n"));
